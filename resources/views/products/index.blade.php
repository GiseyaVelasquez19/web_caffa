...
@section('content')
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        @can('products create')
            <a href="{{ route('products.create') }}" class="text-white font-medium py-2 px-4 rounded-lg transition text-sm whitespace-nowrap" style="background-color: #6F4E37" onmouseover="this.style.backgroundColor='#5A3E2B'" onmouseout="this.style.backgroundColor='#6F4E37'">
                + Nuevo Producto
            </a>
        @endcan
    </div>
            ...
            <table class="w-full min-w-[600px]">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide hidden sm:table-cell">Categoría</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Precio</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide hidden md:table-cell">Stock</th>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-400 sm:hidden">{{ $product->category->nombre ?? '—' }}</p>
                                        <p class="text-xs text-gray-400">{{ $product->codigo ?? 'Sin código' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-500 hidden sm:table-cell">{{ $product->category->nombre ?? '—' }}</td>
